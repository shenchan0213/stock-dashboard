...
def create_intraday_chart(df: pd.DataFrame, title: str) -> Optional[go.Figure]:
    ...
    # 如果收盤價 > 開盤價 = 漲 (用 primary 綠色)
    # 否則 = 跌 (用 danger 紅色)
    if end_price >= start_price:
        line_color = COLORS["primary"] 
        fill_color = COLORS["fill_green"] # 使用半透明綠
    else:
        line_color = COLORS["danger"]
        fill_color = COLORS["fill_red"]   # 使用半透明紅
    ...
    # 1. 價格線 (Area Chart 面積圖)
    fig.add_trace(go.Scatter(
        x=df["Datetime"], y=df["Close"],
        mode="lines", 
        name="PRICE",
        line=dict(color=line_color, width=2),
        fill='tozeroy', # 填滿下方區域，讓視覺更飽滿
